...
def gpu_gadget_worker(local_rank: int, pod_index: int, result_queue) -> None:
    ...
    # Both pods independently repeat a disjoint eight-way partition of base 34.
    base = 34
    target_size = base // 2
    full_mask = (1 << base) - 1
    total_masks = 1 << base
...
def main() -> None:
    ...
    summary = {
        ...
        "gpu_moduli_checked": [34],
        "base34_normalized_candidates": sum(
            row["normalized_candidates"] for row in gpu_rows
        ),
        "base34_sum_full_candidates": sum(
            row["sum_full_candidates"] for row in gpu_rows
        ),
        "base34_minimum_difference_size": global_minimum,
        "base34_minimizer_count": sum(
